...
data = zeros(2,N);
label = zeros(1,N);
%构建双月形状，设定半径、宽度、距离、以及核函数的方差

for i=1:N                           %at the beginning of every round, to produce the training set in random;
    ...
    if  rand>0.5                    %make sure the points occur in random. and half in half for each class;
        ...
        data(1,i)=lenth*cos(theta);          %the X axis;
        data(2,i)=lenth*sin(theta);          %the Y axis;
        data(3,i)=-2*(-lenth*sin(theta))*rand+2;     %
        label(i)=-1;                     %the expected response; class 1;
    else
        beta=-pi*rand;
        len_2=r+wth*(rand-0.5);
        data(1,i)=len_2*cos(beta)+r;
        data(2,i)=len_2*sin(beta)-dis;
        data(3,i)=(5-len_2*sin(beta))*rand;
        label(i)=1;                   %class 2;
    end
end
data = data';
...
neg = find(label == -1);
figure;
plot3(data(pos,1),data(pos,2),data(pos,3),'r+');
hold on;
plot3(data(neg,1),data(neg,2),data(neg,3),'b*');
hold off;
title('训练集 ')
%% 定义向量机参数
alphas = zeros(num_data,1);     %拉格朗日乘子
b = 0;

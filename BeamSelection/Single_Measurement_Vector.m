...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% This scripts generates the results presented in Fig. 11 and Fig. 13 in the manuscript
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% WconjkronZ_m=kron(transpose(W_m),Z_m')';
%% Variables to store results
Rate_exh_sear=zeros(length(Nrx_vec),1); % Rate for exhaustive-search
SP_exh_sear_B1=zeros(length(Nrx_vec),1); % Success percentage for exhaustive-search (B1)
SP_exh_sear_B5=zeros(length(Nrx_vec),1); % Success percentage for exhaustive-search (B5)
%
Rate_OMP=zeros(length(Nrx_vec),1); % Rate for OMP
SP_OMP_B1=zeros(length(Nrx_vec),1); % Success percentage for OMP (B1)
SP_OMP_B5=zeros(length(Nrx_vec),1); % Success percentage for OMP (B5)
%
Rate_StrucLWOMP=zeros(length(Nrx_vec),1); % Rate for structured LW-OMP
SP_StrucLWOMP_B1=zeros(length(Nrx_vec),1); % Success percentage for structured LW-OMP (B1)
SP_StrucLWOMP_B5=zeros(length(Nrx_vec),1); % Success percentage for structured LW-OMP (B5)
...
for ee=1:E
    ...
    %% Omni-directional channel impulse response parameters
    ...
    p=p/(max(max(ZHW_s_abs))-min(min(ZHW_s_abs)));
    p=J_p*p;
    p=p(:);
    %% Randomly chosen subcarrier for sensing
    subcarrier_index=randi(K_m);
    %% Main Loop
    for mm=1:length(Nrx_vec)
        ...
        [ZHW_exh_sear_no_noise_m]=dictionary_operated_channel(H_freq_m,...
            zeros(Mrx_m,Mtx_m,K_m),W_m,Z_m,K_m,subcarrier_index,0);
        ...
        %% Exhaustive Search
        [ZHW_exh_sear_m]=dictionary_operated_channel(H_freq_m,v_m,...
            W_m,Z_m,K_m,subcarrier_index,0);
        ...
        %
        for ii=1:K_m
            Rate_exh_sear(mm)=Rate_exh_sear(mm)+...
                1/E*1/K_m*log2(1+abs(Z_m(:,I_exh_sear)'*H_freq_m(:,:,ii)*W_m(:,J_exh_sear))^2*SNR_m/K_m);
        end
        %
        SP_exh_sear_B1(mm)=SP_exh_sear_B1(mm)+1/E*(Result_exh_sear==Best_beam_index);
        SP_exh_sear_B5(mm)=SP_exh_sear_B5(mm)+1/E*length(intersect(Best_five_beams_index,Result_exh_sear));
        %% OMP
        [ZHW_OMP_m]=dictionary_operated_channel(H_freq_m,v_m(:,1:Ntx,:),F(:,1:Ntx),Q(:,1:Nrx),K_m,subcarrier_index,0);
        ...
        %
        Result_OMP=LWOMP(ZHW_OMP_m(:),Psi);
        [I_OMP,J_OMP] = ind2sub(size(ZHW_exh_sear_no_noise_abs_m),Result_OMP);
        %
        for ii=1:K_m
            Rate_OMP(mm)=Rate_OMP(mm)+1/E*1/K_m*log2(1+abs(Z_m(:,I_OMP)'*H_freq_m(:,:,ii)*W_m(:,J_OMP))^2*SNR_m/K_m);
        end
        SP_OMP_B1(mm)=SP_OMP_B1(mm)+1/E*(Result_OMP==Best_beam_index);
        SP_OMP_B5(mm)=SP_OMP_B5(mm)+1/E*length(intersect(Best_five_beams_index,Result_OMP));
        %% SLW-OMP
        [ZHW_StrucLWOMP_m]=dictionary_operated_channel(H_freq_m,v_m(:,1:Ntx,:),F_structured(:,1:Ntx),Q_structured(:,1:Nrx),K_m,subcarrier_index,0);
        ...
        % normlize Psistruc
        Result_StrucLWOMP=LWOMP(ZHW_StrucLWOMP_m(:),Psistruc,p,sqrt(K_m/SNR_m));
        %
        [I_StrucLWOMP,J_StrucLWOMP] = ind2sub(size(ZHW_exh_sear_no_noise_abs_m),Result_StrucLWOMP);
        %
        for ii=1:K_m
            Rate_StrucLWOMP(mm)=Rate_StrucLWOMP(mm)+1/E*1/K_m*log2(1+abs(Z_m(:,I_StrucLWOMP)'*H_freq_m(:,:,ii)*W_m(:,J_StrucLWOMP))^2*SNR_m/K_m);
        end
        SP_StrucLWOMP_B1(mm)=SP_StrucLWOMP_B1(mm)+1/E*(Result_StrucLWOMP==Best_beam_index);
        SP_StrucLWOMP_B5(mm)=SP_StrucLWOMP_B5(mm)+1/E*length(intersect(Best_five_beams_index,Result_StrucLWOMP));
    end
end
%% Figure 12
Tc=Mrx_m*Mtx_m*128;
...
Effective_Rate_OMP=(1-(1./Tc')*(Nrx_vec.*Ntx_vec)).*(ones(length(Tc),1)*Rate_OMP');
Effective_Rate_StrucLWOMP=(1-(1./Tc')*(Nrx_vec.*Ntx_vec)).*(ones(length(Tc),1)*Rate_StrucLWOMP');
%
...
hold on,plot(Nrx_vec.*Ntx_vec,Effective_Rate_StrucLWOMP(1,:),'rd-','linewidth',2);
%
le=legend({'exhaustive-search','OMP','structured LW-OMP'});le.Interpreter='latex';le.Location='southeast';le.FontSize=14;
...
%% Figure 13
figure,plot(Nrx_vec.*Ntx_vec,ones(1,length(Nrx_vec))*mean(SP_exh_sear_B1*100),'k-','linewidth',2);
hold on,plot(Nrx_vec.*Ntx_vec,SP_OMP_B1*100,'bs-','linewidth',2);
hold on,plot(Nrx_vec.*Ntx_vec,SP_StrucLWOMP_B1*100,'rd-','linewidth',2);
%
hold on,plot(Nrx_vec.*Ntx_vec,ones(1,length(Nrx_vec))*mean(SP_exh_sear_B5*100),'k--','linewidth',2);
hold on,plot(Nrx_vec.*Ntx_vec,SP_OMP_B5*100,'bs--','linewidth',2);
hold on,plot(Nrx_vec.*Ntx_vec,SP_StrucLWOMP_B5*100,'rd--','linewidth',2);
%
le=legend({'exhaustive-search','OMP','structured LW-OMP'});le.Interpreter='latex';le.Location='southeast';le.FontSize=14;
xlabel('Measurements ($N_{\mathrm{RX}}\times N_{\mathrm{TX}}$)','Interpreter','latex'),ylabel('$\mathrm{SP}$ ($\%$)','Interpreter','latex')
ax=gca;ax.TickLabelInterpreter='latex';ax.FontSize=14;
box off
grid on
dim = [0.15 0.55 0 0];
str = {'Solid: $\mathcal{B}_1$','Dashed: $\mathcal{B}_5$'};
